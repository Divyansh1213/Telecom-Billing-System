...
void addrecords()
{
	...
	{   f=fopen("c:/file.ojs","wb+");
		...
		printf("/npress any key to proceed");
		getch();
	}
	while(1)
	{
		...
		printf("\n Press esc key to exit, any other key to add another:");
		test=getche();
		if(test==27)
			break;
	}
...
void deleterecords()
{
	...
	exit(0);
	system("cls");
	printf("Enter the phone number that needs to be deleted");
	fflush(stdin);
	scanf("%[^\n]",phonenumber);
	while(fread(&s,sizeof(s),1,f)==1)
	{
...
void changerecords()
{
	...
	if((f=fopen("c:/file.ojs","rb+"))==NULL)
		exit(0);
	system("cls");
	printf("Enter phone number of the subscriber that needs to be changed:");
	scanf("%[^\n]",phonenumber);
	fflush(stdin);
	while(fread(&s,sizeof(s),1,f)==1)
	{
